...
Next, hyperparameter tuning is performed using six tuning hyperparameters to identify the optimal model configuration and improve predictive performance.
The models are then evaluated using appropriate performance metrics, including accuracy, ROC-AUC, and the Brier score (brier_class).
Finally, a brief discussion explains why feature importance analysis is not conducted for ANN models, in contrast to interpretable models such as random forests, highlighting the inherent black-box nature of neural networks.
